...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('neptune/assets/new/blur.png')
...
local hash = loadstring(downloadFile('neptune/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('neptune/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('neptune/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
	...
	function whitelist:update(first)
		...
		local suc = pcall(function()
			...
			whitelist.textdata = game:HttpGet('http://localhost:0/disabled/'..commit..'/PlayerWhitelist.json', true)
		end)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('neptune/profiles/whitelist.json') and readfile('neptune/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('neptune/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('neptune/assets/new/arrowmodule.png')
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('neptune/assets/new/radaricon.png'),
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'neptune on top')
...
-- ============================================================
-- Neptune — Custom Modules (Permission Spoof, Studio Spoof, Anti Reporter,
-- Player Lookup IDOR, Admin Panel, God Mode)
-- ============================================================

run(function()
	local hookmetamethod = hookmetamethod or getgenv().hookmetamethod
	local getnamecallmethod = getnamecallmethod or getgenv().getnamecallmethod
	local newcclosure = newcclosure or function(f) return f end
	local cloneref = cloneref or function(o) return o end

	-- ===== Permission Spoof =====
	local PERMS_LIST = {"admin","anticheat_mod","artist","director","filmer","mod","all_kits","all_kit_skins","ranked_skins_bypass","tester","freecam"}
	local SpoofPerms
	local rs = cloneref(game:GetService("ReplicatedStorage"))

	local function tryPatchController()
		local ok, KnitClient = pcall(function()
			return require(rs.rbxts_include.node_modules["@easy-games"].knit.src).KnitClient
		end)
		if not ok or not KnitClient or not KnitClient.Controllers then return false end
		local pc = KnitClient.Controllers.PermissionController
		if not pc then return false end
		pc.permissions = PERMS_LIST
		pc.isReady = true
		local origGet = pc.getLocalPermissions
		pc.getLocalPermissions = function(self)
			if SpoofPerms and SpoofPerms.Enabled then return PERMS_LIST end
			return origGet(self)
		end
		local origWait = pc.waitForLocalPermissions
		pc.waitForLocalPermissions = function(self)
			if SpoofPerms and SpoofPerms.Enabled then return PERMS_LIST end
			return origWait(self)
		end
		return true
	end

	SpoofPerms = vape.Categories.Render:CreateModule({
		Name = "Permission Spoof",
		Tooltip = "Tricks the CLIENT UI into having admin / all_kits / all_kit_skins / freecam perms. Unlocks try-on, freecam button, streamer-mode see-through. Server-validated actions still rejected.",
		Function = function(callback)
			if callback then
				task.spawn(function()
					local deadline = tick() + 30
					while tick() < deadline and SpoofPerms.Enabled do
						if tryPatchController() then break end
						task.wait(0.5)
					end
				end)
			end
	...
	local RunService = cloneref(game:GetService("RunService"))
	local origIsStudio
	StudioSpoof = vape.Categories.Render:CreateModule({
		Name = "Studio Spoof",
		Tooltip = "Hooks RunService:IsStudio() to return true. Unlocks lobby script editor, debug menus, reduced AFK timer.",
		Function = function(callback)
			if callback then
				if hookmetamethod and getnamecallmethod and not origIsStudio then
					origIsStudio = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
						local m = getnamecallmethod()
						if m == "IsStudio" and self == RunService then return true end
						return origIsStudio(self, ...)
					end))
				end
			end
		end,
	...
	-- ===== Anti Reporter =====
	local AntiReport
	local BLOCK_SUBSTRINGS = {"TabFreezeAnticheat","AnticheatReport","CheatReport","ExploitDetected","ReportPlayer","ReportSubmit","SuspiciousAction","IntegrityCheck"}
	local antiReportHooked = false
	local origNamecallAR
	AntiReport = vape.Categories.Render:CreateModule({
		Name = "Anti Reporter",
		Tooltip = "Drops any FireServer/SendToServer/InvokeServer whose remote name matches known client-side tattletale patterns. Default ON.",
		Function = function(callback)
			if callback then
				if hookmetamethod and getnamecallmethod and not antiReportHooked then
					origNamecallAR = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
						local m = getnamecallmethod()
						if (m == "FireServer" or m == "SendToServer" or m == "InvokeServer") and typeof(self) == "Instance" then
							local n = self.Name
							for _, sub in ipairs(BLOCK_SUBSTRINGS) do
								if n:find(sub) then return nil end
							end
						end
						return origNamecallAR(self, ...)
					end))
					antiReportHooked = true
				end
			end
		end,
	})
...
run(function()
	-- ===== Player Lookup (RequestMatchHistory IDOR) =====
	local cloneref = cloneref or function(o) return o end
	local rs = cloneref(game:GetService("ReplicatedStorage"))
	local httpService = cloneref(game:GetService("HttpService"))
	local lookupOk, remotes = pcall(function() return require(rs.TS.remotes).default end)

	local Lookup
	local input = ""

	local function notify(...) return vape:CreateNotification(...) end

	local function resolveUserId(query)
		query = tostring(query):gsub("^%s+", ""):gsub("%s+$", "")
		if query:match("^%d+$") then return tonumber(query) end
		local ok, body = pcall(function()
			return httpService:PostAsync(
				"https://users.roblox.com/v1/usernames/users",
				httpService:JSONEncode({usernames = {query}, excludeBannedUsers = false}),
	...
	local function run_lookup(query)
		if not lookupOk or not remotes then return notify("Lookup", "remotes module not available", 5, "alert") end
		local uid, err = resolveUserId(query)
		if not uid then return notify("Lookup", "resolve fail: " .. tostring(err), 5, "alert") end
		local r = remotes.Client:Get("RequestMatchHistory")
		local ok, ret = pcall(function() return r:CallServer(uid) end)
		if not ok or type(ret) ~= "table" or not ret.matchHistory then
			return notify("Lookup", "remote rejected (no record for " .. tostring(uid) .. ")", 6, "alert")
		end
		local mh = ret.matchHistory
		local wins, losses, kills, deaths = 0, 0, 0, 0
		for _, m in ipairs(mh) do
			for _, p in ipairs(m.players or {}) do
				if p.userId == uid or (p.playerInfo and p.playerInfo.userId == uid) then
					local g = p.generic or {}
					kills = kills + (g.kills and g.kills.total or 0)
					deaths = deaths + (g.deaths or 0)
					if g.matchOutcome == "win" then wins = wins + 1
					elseif g.matchOutcome == "loss" then losses = losses + 1 end
					break
				end
			end
		end
		notify("Lookup: " .. (ret.player and ret.player.name or uid),
			("Last " .. #mh .. " matches | W " .. wins .. " / L " .. losses .. " | K " .. kills .. " / D " .. deaths),
			15, "info")
		print("[Axol Lookup] full payload for", uid, ":")
		print(httpService:JSONEncode(ret):sub(1, 4000) .. " ...")
	end

	Lookup = vape.Categories.Render:CreateModule({
		Name = "Player Lookup",
		Tooltip = "Pulls any player's last-10 bedwars match history via RequestMatchHistory IDOR.",
		Function = function(callback)
			if callback then
				if input == "" then notify("Lookup", "Type a username or userId first", 5, "alert")
				else task.spawn(run_lookup, input) end
				task.defer(function() if Lookup.Enabled then Lookup:Toggle() end end)
			end
		end,
	})
	Lookup:CreateTextBox({
		Name = "Username or UserId",
		Function = function(text) input = text end,
		Placeholder = "e.g. SomeUser or 4596790442",
	})
end)
...
run(function()
	...
	local isfile_ = isfile or function(p) local ok, r = pcall(readfile, p); return ok and r ~= nil and r ~= "" end
	local writefile_ = writefile or function() end
	local readfile_ = readfile or function() return "" end

	local BAN_FILE = "neptune/profiles/banlist.json"
	local function loadBans()
		if not isfile_(BAN_FILE) then return {} end
		local ok, t = pcall(function() return httpService:JSONDecode(readfile_(BAN_FILE)) end)
		return (ok and type(t) == "table") and t or {}
	end
	local function saveBans(b)
		pcall(function() writefile_(BAN_FILE, httpService:JSONEncode(b)) end)
	end
	local bans = loadBans()
	local hidden = {}
	...
	local function resolvePlayer(query)
		...
		if query:lower() == "all" then return "all" end
		...
		if uid then for _, p in ipairs(Players:GetPlayers()) do if p.UserId == uid then return p end end end
		return nil
	end

	local function sendChatCmd(cmd)
	...
	local function reportSpam(target, count)
		count = count or 40
		local ok, rem = pcall(function() return require(rs.TS.remotes).default end)
		if not ok or not rem then return false end
		local rp
		ok, rp = pcall(function() return rem.Client:Get("ReportPlayer") end)
		if not ok or not rp then return false end
		local uid = type(target) == "number" and target or target.UserId
		for i = 1, count do
			pcall(function() rp:SendToServer(uid) end)
			task.wait(0.03)
		end
		return true
	end
	...
	local function unhidePlayer(plr)
		if not plr or not plr.Character then return end
		for _, d in ipairs(plr.Character:GetDescendants()) do
			if d:IsA("BasePart") then
	...
	local function doKick(plr, reason)
		if not plr or plr == lplr then return end
		sendChatCmd("/kick " .. plr.Name .. (reason and reason ~= "" and " " .. reason or ""))
		task.spawn(function() reportSpam(plr, 40) end)
		hidePlayer(plr)
	end

	local function doBan(plr, reason)
		if not plr or plr == lplr then return end
		bans[tostring(plr.UserId)] = {name = plr.Name, time = os.time(), reason = reason or ""}
		saveBans(bans)
		doKick(plr, reason)
	end

	local function applyForAll(fn, ...)
		for _, p in ipairs(Players:GetPlayers()) do
			if p ~= lplr then fn(p, ...) end
		end
	end

	Players.PlayerAdded:Connect(function(p)
		task.wait(2)
		if bans[tostring(p.UserId)] then
			notify("AdminPanel", "Banned player joined: " .. p.Name, 5, "alert")
			doKick(p, bans[tostring(p.UserId)].reason or "auto-ban")
		end
	end)
	task.defer(function()
		for _, p in ipairs(Players:GetPlayers()) do
			if p ~= lplr and bans[tostring(p.UserId)] then doKick(p, "auto-ban") end
		end
	end)

	local Panel = vape.Categories.Render:CreateModule({
		Name = "Admin Panel",
		Tooltip = "Kick/Ban/Unban with chat-cmd attempt + report-spam fallback + persistent ban list.",
		Function = function() end,
	})
	local currentTarget = ""
	local currentReason = ""
	local reportCount = 40

	Panel:CreateTextBox({
		Name = "Target",
		Placeholder = 'name / displayname / userId / "all"',
		Function = function(t) currentTarget = t end,
	})
	Panel:CreateTextBox({
		Name = "Reason",
		Placeholder = "(optional)",
		Function = function(t) currentReason = t end,
	})
	Panel:CreateSlider({
		Name = "Report Spam Count",
		Min = 1, Max = 200, Default = 40,
		Function = function(v) reportCount = v end,
	})

	local function singleShotToggle(name, fn, tooltip)
		local tog
		tog = Panel:CreateToggle({
			Name = name,
			Tooltip = tooltip,
	...
	singleShotToggle("KICK", function()
		local p = resolvePlayer(currentTarget)
		if not p then return notify("Kick", "No player", 4, "alert") end
		if p == "all" then applyForAll(doKick, currentReason); return notify("Kick", "Kicked all", 4, "info") end
		doKick(p, currentReason)
		notify("Kick", "Sent /kick + 40 reports + hidden: " .. p.Name, 5, "info")
	end, "Try /kick (host-only) + flood reports + hide locally.")

	singleShotToggle("BAN", function()
		local p = resolvePlayer(currentTarget)
		if not p then return notify("Ban", "No player", 4, "alert") end
		if p == "all" then applyForAll(doBan, currentReason); return notify("Ban", "Banned all", 4, "info") end
		doBan(p, currentReason)
		notify("Ban", p.Name .. " banned + persisted", 5, "info")
	end, "Kick + persist to local banlist.json. Auto-applies on their rejoin.")

	singleShotToggle("UNBAN", function()
		local q = tostring(currentTarget):lower()
		if q == "all" then bans = {}; saveBans(bans); return notify("Unban", "Cleared all bans", 4, "info") end
		local removed = 0
		for uid, info in pairs(bans) do
			if uid == q or (info.name and info.name:lower() == q) then bans[uid] = nil; removed = removed + 1 end
		end
		if removed > 0 then saveBans(bans); notify("Unban", "Removed " .. removed .. " ban(s)", 4, "info")
		else notify("Unban", "Not in ban list", 4, "alert") end
	end, "Remove userId / name from ban list.")

	singleShotToggle("Report Spam", function()
		local p = resolvePlayer(currentTarget)
		if not p then return notify("Report", "No player", 4, "alert") end
		if p == "all" then applyForAll(function(pp) reportSpam(pp, reportCount) end); return notify("Report", "All x" .. reportCount, 4, "info") end
		task.spawn(reportSpam, p, reportCount)
		notify("Report", reportCount .. " queued for " .. p.Name, 4, "info")
	end, "Bypass client throttle and flood ReportPlayer remote.")

	singleShotToggle("Hide Player", function()
		local p = resolvePlayer(currentTarget)
		if p == "all" then applyForAll(hidePlayer); return notify("Hide", "Hid all", 3, "info") end
		if p then hidePlayer(p); notify("Hide", p.Name, 3, "info") end
	end, "Local invisibility + no collision.")

	singleShotToggle("Unhide Player", function()
		local p = resolvePlayer(currentTarget)
		if p == "all" then applyForAll(unhidePlayer); return notify("Unhide", "Restored", 3, "info") end
		if p then unhidePlayer(p); notify("Unhide", p.Name, 3, "info") end
	end, "Restore visibility.")

	singleShotToggle("List Bans", function()
		local lines = {}
		for uid, info in pairs(bans) do
			table.insert(lines, string.format("%s (%s)", info.name or "?", uid))
		end
		if #lines == 0 then table.insert(lines, "(empty)") end
...
run(function()
	...
	local God
	local cfg = {walkSpeed = 35, jumpPower = 75, maxHealth = 9999, applySpeed = true, applyJump = true, applyHealth = true, forceField = false, safe = true}
	local SAFE_SPEED_MAX, SAFE_JUMP_MAX = 60, 150
	local hbConn, ffInstance

	local function apply(char)
		if not char then return end
		local hum = char:FindFirstChildOfClass("Humanoid")
		if not hum then return end
		local s, j = cfg.walkSpeed, cfg.jumpPower
		if cfg.safe then
			if s > SAFE_SPEED_MAX then s = SAFE_SPEED_MAX end
			if j > SAFE_JUMP_MAX then j = SAFE_JUMP_MAX end
		end
		if cfg.applySpeed and hum.WalkSpeed ~= s then hum.WalkSpeed = s end
		if cfg.applyJump and hum.JumpPower ~= j then hum.JumpPower = j end
		if cfg.applyHealth then
			if hum.MaxHealth ~= cfg.maxHealth then hum.MaxHealth = cfg.maxHealth end
			if hum.Health < cfg.maxHealth then hum.Health = cfg.maxHealth end
		end
		if cfg.forceField then
			if not (ffInstance and ffInstance.Parent == char) then
				ffInstance = Instance.new("ForceField")
				ffInstance.Visible = true
				ffInstance.Parent = char
			end
		else
			if ffInstance then ffInstance:Destroy(); ffInstance = nil end
		end
	end

	local function start()
		if hbConn then return end
		hbConn = RunService.Heartbeat:Connect(function() apply(lplr.Character) end)
	end
	local function stop()
		...
		if hum then hum.WalkSpeed = 16; hum.JumpPower = 50; hum.MaxHealth = 100 end
	end

	God = vape.Categories.Blatant:CreateModule({
		Name = "God Mode",
		Tooltip = "Force-writes Humanoid.WalkSpeed/JumpPower/MaxHealth/Health every Heartbeat.",
		Function = function(state)
			if state then start() else stop() end
		end,
	})
	God:CreateSlider({Name = "Walk Speed", Min = 16, Max = 250, Default = 35, Function = function(v) cfg.walkSpeed = v end})
	God:CreateSlider({Name = "Jump Power", Min = 50, Max = 500, Default = 75, Function = function(v) cfg.jumpPower = v end})
	God:CreateSlider({Name = "Max Health", Min = 100, Max = 99999, Default = 9999, Function = function(v) cfg.maxHealth = v end})
	God:CreateToggle({Name = "Apply Speed", Enabled = true, Function = function(s) cfg.applySpeed = s end})
	God:CreateToggle({Name = "Apply Jump", Enabled = true, Function = function(s) cfg.applyJump = s end})
	God:CreateToggle({Name = "Apply Health", Enabled = true, Function = function(s) cfg.applyHealth = s end})
