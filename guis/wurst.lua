...
local getcustomassets = {
	['neptune/assets/wurst/triangle.png'] = 'rbxasset://wurst/triangle.png'
}
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/UdAxol/Neptune/'..readfile('neptune/profiles/commit.txt')..'/'..select(1, path:gsub('neptune/', '')), true) end)
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:CreateModule(modulesettings)
		...
		triangle.Image = getcustomasset('neptune/assets/wurst/triangle.png')
...
for _, v in {'Combat', 'Blatant', 'Render', 'Utility', 'World', 'Inventory', 'Minigames', 'Modules'} do
